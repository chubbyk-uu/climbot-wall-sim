...
`1.87 GB`），余下的按每 worker `120 MB` 分配；预算不足以覆盖固定开销时退化为单进程。

每个渲染 worker 有一个严格限制为 `32 MiB` 的解码图像 LRU，并按连续 tile 小块调度；两个
全分辨率 pass 共用同一批 worker。这样只改变 PNG 解码次数和耗时，不改变 row-major 写入顺序
或任何正式产物的像素。`mosaic_manifest.json` 的每个 pass 都记录命中/未命中、缓存上限、单个
worker 峰值缓存、聚合 PNG 解码 CPU 时间和 tile 块大小，便于后续 CPU/CUDA 对照。
...
域外缺口同时按两套冻结几何分类。`planned_scan_footprints_m` 是沿任务中每条 SCAN 线连续
扫出的实际计划足迹；`safe_pose_camera_envelopes_m` 是相机位于 `motion_region` 内、朝任务允许
的正反扫描方向时的可观测域。前者回答“这份计划本应扫到哪里”，后者为“存在安全中心位姿
可以拍到”提供几何见证。两者不再用从 `coverage_region` 膨胀出的保守上包络代替。
