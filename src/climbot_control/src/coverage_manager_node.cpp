...
private:
  ...
  {
    // A finalization timeout or local inability to finalize is the manager's
    // terminal verdict for this run. The recorder can still publish a status
    // generated by the retired RPC; accepting it would undo that verdict even
    // though the service Future itself is protected by its generation. A
    // normal response does not retire this topic: its terminal status carries
    // the recorder's final image counts and may cross the service reply in DDS.
    if (archive_status_retired_ || !status_.inspection_enabled ||
      archive.task_id != status_.task_id ||
  ...
  bool archive_status_retired_{false};
