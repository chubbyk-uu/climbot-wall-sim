...
恢复时**期限平移，曲线重规划**，两者要分开看。

期限（任务耗时、段超时、入轨超时、转向稳定窗口）按真正静止的那段时长整体平移，所以暂停不会
触发段超时。被平移的只有静止那一段——减速阶段机器人还在本任务下行走，那部分时间照算。

运动曲线不能这么处理。行进曲线和转向曲线都是**从静止起算的时间参数化曲线**，把它从中段接着
放，等于要求机器人此刻已经在巡航速度上，而它是零。`time` 模式的调度修正会把这个差读成滞后，
然后按 `catch_up_max_linear_speed` 去追——实测恢复瞬间命令速度 `0.350 m/s`，而暂停前是
`0.203 m/s`。减速还会先垫一笔：曲线按巡航推进了整个刹车时间，机器人只走了一半距离
（`v²/2a`，按当前配置约 `0.08 m`），`time_along_gain` 取 2.5 时光这一项就是 `0.20 m/s`。

所以恢复时直接从当前位姿重新规划，跟一个段刚开始时做的事完全一样：行进段调用 `beginTravel()`
重排梯形/三角形曲线并清零滞后与积分；转向途中（`ALIGN_PROFILE`）退回 `ALIGN_BRAKE`，让对齐
逻辑按剩余航向误差重排一条新的转向曲线。`distance` 模式从不读这两条曲线，所以它一直都是干净
恢复的。
