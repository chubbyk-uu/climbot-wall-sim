...
一次渲染曝光是 `1920 x 1080 x 3 = 6220800` 字节，而 Fast DDS 默认的共享内存段约为 512 KiB。
图像会按 `maxMessageSize` 分成 RTPS 分片；默认段连一帧的全部分片都无法同时保留，持续触发下
写端会耗尽段内缓冲、丢掉一个分片，可靠读端要等到三秒后的下一次周期心跳才发现缺口——这就是
偶发出图停顿的根因。`climbot_common/config/fastdds_inspection_image.xml` 把段容量放大到 64 MiB，
并且只挂给三个大图发布参与者：`inspection_camera_bridge`、`camera_distortion_adapter` 和
`capture_once_node`（见 `additional_env`）。其他参与者无需为该段付内存代价。环境里已有
...
- `src/total_station_sim_node.cpp`：12 Hz 带噪声和延迟的绝对位置；
...
- `climbot_gazebo/wall_texture.py`：把烘焙好的贴图块作为纯视觉 visual 挂到墙面上；碰撞盒与摩擦参数不动。贴图块的**外表面必须落在碰撞面上**，因为轮子压的和相机拍的必须是同一个平面：`0.275 m` 物距上哪怕 `1 mm` 的偏差，都会让离线拼接整幅带上 `+3600 ppm` 量级的尺度误差。为此 `wall_visual` 在 `climbot_wall.sdf.xacro` 里内缩让开 z-fighting，见 `SURFACE_OFFSET_M` 的注释。烘焙工具在 `tools/`，产物不进仓库，用法见 [OPERATION.md](../../docs/OPERATION.md) 的“墙面贴图”。
