...
- **橙色四边形**是点选出来的作业区域，`A`/`B`/`C`/`D` 是四个顶点；
- **蓝色折线**是规划出的弓字扫描路径，机器人正走在第 5 段；
...
- 阶段 F：系统测试与数据评价——进行中（覆盖基线与 §14.4 侧滑补偿专项验收已产出，当前基线是哪一批、覆盖哪些工况见 [results/README.md](results/README.md)；单段直线、定位噪声和固定种子重复性三项测试场景待补）。
