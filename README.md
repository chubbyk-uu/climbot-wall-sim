...
> **标定基线已失效，待重新测量。** 下列数值是质心高度下调（`base_link` z 由 `0.15` 降至 `0.085`）之前测得的。该改动使下行工况的主动轮法向载荷由 `102.7 N` 变为 `91.2 N`，纵向附着能力随之改变，因此这组数字不能再用作回归对比。
...
上下行速度差的主因是**法向载荷转移**而非 WheelSlip 参数本身：下行时重力把车头压向墙面、把车尾撬离墙面，主动轮法向载荷显著高于上行。实测数据见 `results/`。
