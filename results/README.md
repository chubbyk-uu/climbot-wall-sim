...
| 执行时间 | 242.684 s | 284.711 s |
| 实际覆盖率 | 98.667% | 98.123% |
| 漏扫比例 | 1.333% | 1.877% |
| 最大单段横轨 RMS | 1.95 mm | 2.91 mm |
| 最大绝对横轨误差 | 9.48 mm | 4.26 mm |
| 最大动态终点误差 | 2.93 mm | 3.57 mm |
| 最大转向结束航向误差（真值口径） | 1.75° | 1.85° |
| 最大扫描线偏离名义 | 11.34 mm | 8.92 mm |
| 最大相邻扫描线间距误差 | 18.37 mm | 9.70 mm |
| 最大水平扫描净高度误差 | 2.40 mm | 不适用 |
| 实际/名义线段总长比 | 1.0454 | 1.0232 |
| 最大机体航向补偿角 | 6.73° | 7.01° |
| 正式直线最大指令角速度 | 0.072 rad/s | 0.073 rad/s |
| 可见反向往复 | 0 | 0 |

横向与竖向梯形均超过 `98%` 门限，因此本版不增加顶部收边扫描。竖向结果仅高出
门限 `0.123` 个百分点；修改检测足迹、行距、转向下滑或入轨参数后必须重新评价，
不能把”不需要收边”视为对所有梯形永久成立。

**覆盖率余量比上一版更薄，这是终点停车精度修复的代价，不是回归。** 修复前扫描线
带有 `+12~+23 mm` 的系统性偏置，恰好把上部各线推向梯形较窄的一侧、把首线压低，
额外扫到了一些边缘面积；三份基线的覆盖率因此偏高（横向 `99.312%`、竖向
`98.186%`）。现在扫描线落在名义位置附近，覆盖率回到规划路径本身所能提供的水平。
真正的余量来源是顶部收边扫描（尚未实现）和规划期覆盖率门限的保守化，不是执行
误差；竖向梯形 `0.123` 个百分点的余量应视为该项工作的触发条件。
