...
| P2-06 联合盲测 | [mosaic_p206_blind_2026-08-30b_summary.json](mosaic_p206_blind_2026-08-30b_summary.json) | **严格证据链重生成中，当前暂不引用。** 测量已逐位复现，但旧摘要尚未记录生成器自身 provenance，且域外可达性使用了只能作上界的包络；域内零漏拍结论待新摘要重新发布 |
...
- 巡检域外有 `439,116` 个未覆盖像素。旧分类只证明它们都落在保守上包络内，不能据此断言
  每个像素存在合法位姿；冻结计划足迹与安全位姿包络的严格分类正在重新生成。
