...
> 18 个标签里只有 `2026-08-17`、`timeA`、`ceilgate2` 三个是在干净工作树上产出的，
> 包括本文上面指定为"当前正式基线"的 `2026-08-18d` 在内，其余 15 个都带未提交改动。
> 见下方标签索引的"可追溯"一列。回归脚本现在会在开跑前检查 `src/`，脏树时给标签
> 加 `-dirty` 后缀并告警，评价器也会把 `provenance.git.traceable` 写进摘要并在
> 结束时打印——但这些只对**今后**的运行有效，已归档的 15 个标签无法补救。
...
是因为上面的文件表只覆盖到 `2026-08-18d`，而目录里三分之二的文件属于下面这些
...
| `timeE` | 8 | 08-18 | `4321cd7` | 否 | `P=2.5` 定版回归，`act/plan` `0.981~1.015` |
