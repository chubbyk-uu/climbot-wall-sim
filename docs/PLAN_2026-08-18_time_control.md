...
time_profile_deceleration: 0.20
time_speed_lag_s: 0.08               # 加速度前馈，实测 0.0799 s
time_along_gain: 3.0                 # 纯 P，见 8.7
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| bigV | 3.47 | 0.925 | 5.16 | 100.00 | 1.017 | 0.04 |
| bigTV | 4.59 | 0.893 | 4.48 | 98.99 | 0.998 | 0.03 |
| vertical | 3.94 | 0.864 | 3.20 | 100.00 | 1.007 | 0.03 |
| trapezoid_vertical | 2.96 | 0.880 | 3.20 | 98.63 | 1.003 | 0.05 |
| bigH | 2.91 | 0.749 | 2.26 | 99.97 | 1.010 | 0.03 |
| bigTH | 3.65 | 0.868 | 1.18 | 99.33 | 1.016 | 0.03 |
| trapezoid_horizontal | 3.65 | 0.773 | 4.24 | 99.75 | 1.008 | 0.04 |
| horizontal | 2.80 | 0.773 | 3.57 | 99.94 | 0.991 | 0.03 |
...
在 `bigH / vertical / trapezoid_horizontal / horizontal` 四个用例上扫三个梯度：

| gain | vertical | 峰值滞后 s | 落点 mm | 指令抖动 mean·\|dv/dt\| |
| ---: | --- | ---: | --- | ---: |
| 0.0 | **超时失败**，174 mm / 覆盖 14.6% | 0.88 | — | — |
| 1.0 | 通过 | 0.04~0.06 | 2.50~3.90 | 0.026 |
| 3.0 | 通过 | 0.03 | 2.51~2.89 | 0.069 |
| 6.0 | 通过 | 0.03 | 2.71~3.28 | 0.145 |
...
- **P = 0 直接失败**。线速度通道纯开环，竖向爬升被重力吃掉沿轨距离，曲线走完时
  ...
  **P 项是让段能够结束的东西，不是修饰。**
- **P = 6 相对 P = 3 没有任何收益**（滞后相同、落点不更好），指令抖动却再翻一倍。

拐点在 **P = 3**。选它的依据不是验收指标（那几项在 P=1 已经贴地板），而是把
P=0 暴露出的那个唯一已知失效模式的抗扰余量翻倍。
