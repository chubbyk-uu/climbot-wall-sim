...
| --- | --- | --- | --- |
| G2-01 | 仅正式扫描拍照 | 起点进入、ALIGN、转向稳定、TRANSITION、小弧线入轨和任务停止均为 0 张；只在 SCAN 直线产生图像 | **通过**：三种正式工况均只在 3 条 SCAN 段产生图像，共 18 张／工况 |
| G2-02 | 前后重叠 | 首尾都有拍摄中心；同一扫描线曝光位置最大间距 ≤`0.225 m`，不重复编号、不漏编号 | **通过**：横／竖／梯形最大间距 `208.73/188.64/190.42 mm`，编号与期望张数完整 |
| G2-03 | 左右重叠与覆盖报告 | 相邻相机中心扫描线最大间距 ≤`0.400 m`；记录矩形横／竖和短顶边梯形的离散实际照片足迹覆盖率。普通任务不设面积硬门限，合同验收另行冻结 | **历史结果**：旧“检查区域”语义下最大横向间距 `223.06/373.31/220.47 mm`，离散照片覆盖率 `99.811/99.944/99.815%`；可走区语义下需重新回归 |
| G2-04 | 图像与任务唯一绑定 | 每张图像恰有一条同时间戳元数据；任务 ID／版本、段号和触发号正确，取消／重启不串任务 | **通过**：三工况均 18 对图像／元数据且时间戳完全一致；节点测试覆盖去重、临时拒绝重试和参考变更拒绝 |
...
均为 `20%`，对应名义前后间距不超过 `0.225 m`、左右间距不超过 `0.400 m`。上述
`2026-08-24` 正式摘要使用的是当时的 `25%` 名义策略，仍是该历史配置的有效证据，
但不能作为当前 `20%` 默认策略的动态验收；后者必须重跑 G2 后再登记结果。
