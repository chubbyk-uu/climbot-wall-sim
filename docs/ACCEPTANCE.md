...
| C-09 | 相机本体和支架进入真实惯性模型 | 两部分均有质量、质心、惯量和安装位姿，或有等价刚性总成；自动计算整机总质量、质心和惯量，禁止仅加 visual 或把质量无依据放在光心 | 待实施 |
| C-10 | 多航向载荷不破坏吸附 | 静态 `0°～345°` 每 `15°` 扫描，另测四方向直行／制动和双向原地转向；三点无持续失载，最坏后球轮法向力 ≥`0.15 F_s`，并重跑代表性横／竖任务及全航向转向侧滑 | 待实施 |
| C-11 | 参数与标定可追溯 | 摘要记录内外参、畸变模型、有效 ROI、相机／支架质量和质心、整机质心、配置来源、提交和工作树状态；真机标定不得沿用仿真 `D` 冒充测量值 | 待实施 |
