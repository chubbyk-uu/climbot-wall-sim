...
| C-10 | 多航向载荷不破坏吸附 | 静态 `0°～345°` 每 `15°` 扫描，另测四方向直行／制动和双向原地转向；三点无持续失载，最坏后球轮法向力 ≥`0.15 F_s`，并重跑代表性横／竖任务及全航向转向侧滑。侧滑图要求 48 条齐全、最大值 ≤`0.55 mm/°`、全航向极差 ≤`0.10 mm/°`、最大转角误差 ≤`2°`；其中绝对上限低于现用 `0.41 mm/°` 标定值的 `+50%` 过期告警线 | **部分通过**：102 条载荷记录无失载，最坏后球轮 `75.610 N > 60 N`；横／竖任务已通过，侧滑正式复测待跑 |
