...
## Phase G2 位置触发与位姿绑定验收（旧外参已通过，新外参动态重验中）

| ID | G2 要求 | 验收方法／门限 | 当前状态 |
| --- | --- | --- | --- |
| G2-01 | 仅正式扫描拍照 | 起点进入、ALIGN、转向稳定、TRANSITION、小弧线入轨和任务停止均为 0 张；只在 SCAN 直线产生图像，且每个冻结 SCAN 的实际张数严格等于其位置触发计划 | **历史结果**：旧配置为 3 条 SCAN、18 张／工况；当前 `20%` 名义配置的段数和张数由冻结参考推导，须重跑登记 |
| G2-02 | 前后重叠 | 首尾都有拍摄中心；同一扫描线曝光位置最大间距 ≤`0.2390625 m`（实际重叠 ≥`15%`），不重复编号、不漏编号 | **历史结果**：旧 `25%` 名义配置下横／竖／梯形最大间距 `208.73/188.64/190.42 mm`；当前策略须重新回归 |
| G2-03 | 左右重叠与覆盖报告 | 相邻相机中心扫描线最大间距 ≤`0.425 m`（实际重叠 ≥`15%`）；记录矩形横／竖和短顶边梯形的离散实际照片足迹覆盖率。普通任务不设面积硬门限，合同验收另行冻结 | **历史结果**：旧“检查区域”语义下最大横向间距 `223.06/373.31/220.47 mm`，离散照片覆盖率 `99.811/99.944/99.815%`；可走区语义下需重新回归 |
| G2-04 | 图像与任务唯一绑定 | 每张图像恰有一条同时间戳元数据；任务 ID／版本、段号和触发号正确，取消／重启不串任务 | **历史结果**：旧配置三工况均为 18 对图像／元数据且时间戳完全一致；节点测试覆盖去重、临时拒绝重试和参考变更拒绝 |
| G2-05 | 曝光时刻融合位姿 | EKF 缓存前后插值，不允许最近样本替代；相机中心对真值误差 ≤`5 mm`，航向误差沿用定位验收门限 | **通过**：最大位置误差 `3.544/4.999/4.116 mm`，最大航向误差 `0.189/0.170/0.129°`；竖向位置仅余 `0.0008 mm` 裕量，列为后续优化风险 |
| G2-06 | 相机前置偏移语义一致 | 规划、执行参考、触发、覆盖评价均使用 `+0.340 m`；偏移与安装外参不一致时明确拒绝自动采集 | **代码与节点测试通过，动态重验待办**：共享 YAML 驱动外参，旧 `0.300 m` 正式结果不得用于新机械基线 |
...
`20%` 实际合同相同的 `14.0625 mm` 纵向和 `25 mm` 横向误差裕度。上述 `2026-08-24`
正式摘要使用的是当时的 `25%` 名义策略，仍是该历史配置的有效证据，但不能作为当前
`20%` 默认策略的动态验收；后者必须重跑 G2 后再登记结果。
这些摘要使用当时的 `0.300 m` 外参。为消除原始图底部机体遮挡，光心已前移到
`0.340 m`；因此摘要仍是历史回归证据，但在横／竖／梯形按新外参重跑前，不作为当前
机械基线的最终动态关闭证据。
