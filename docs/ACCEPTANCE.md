...
| C-02 | 前置安装且无遮挡 | 镜头中心外参为 `[0.300,0,0.275] m`；解析净空不小于 `20 mm`，完整有效 ROI 的 Gazebo 图像中不得出现机体、支架或线缆 | **通过**：解析视锥净空通过；1,607,052 像素内缩有效 ROI 经 1 px 抗锯齿边缘容差后未知像素为 0 |
...
| G2-06 | 相机前置偏移语义一致 | 规划、执行参考、触发、覆盖评价均使用 `+0.300 m`；偏移与安装外参不一致时明确拒绝自动采集 | **通过**：共享 YAML 驱动外参，规划与评价采用相机中心；不一致和冻结参考变化均由节点测试明确拒绝 |
