...
| G2-02 | 前后重叠 | 首尾都有拍摄中心；同一扫描线曝光位置最大间距 ≤`0.2390625 m`（实际重叠 ≥`15%`），不重复编号、不漏编号 | **通过**：横／竖／梯形最大间距 `203.594/203.366/210.070 mm` |
| G2-03 | 左右重叠与覆盖报告 | 相邻相机中心扫描线最大间距 ≤`0.425 m`（实际重叠 ≥`15%`）；记录矩形横／竖和短顶边梯形的离散实际照片足迹覆盖率。普通任务不设面积硬门限，合同验收另行冻结 | **通过**：最大横向间距 `415.252/397.598/414.788 mm`；离散照片覆盖率 `81.167/84.333/78.900%` 已记录（可走区语义，无面积硬门限） |
| G2-04 | 图像与任务唯一绑定 | 每张图像恰有一条同时间戳元数据；任务 ID／版本、段号和触发号正确，取消／重启不串任务 | **通过**：三工况 `10/12/9` 对图像／元数据完全同时间戳，编号完整；节点测试覆盖去重、临时拒绝重试和参考变更拒绝 |
| G2-05 | 曝光时刻融合位姿 | EKF 缓存前后插值，不允许最近样本替代；相机中心对真值误差 ≤`5 mm`，航向误差 ≤`1°` | **通过**：最大位置误差 `3.139/3.368/2.721 mm`，最大航向误差 `0.127/0.106/0.198°` |
...
`g2_accept_{horizontal,vertical,trapezoid}_2026-08-25_summary.json`，由
`commit=f52a999` 的干净源码树产生。`2026-08-24` 的三份 `25%` 名义、`0.300 m`
...
| G4-05 | 临时采图故障不损失重叠 | 同一冻结 SCAN 内服务暂拒或图像超时后重试同一目标；相机沿轨位置不得越过待拍目标 `15 mm`，且最终归档仍满足实际间距／后滞门 | **通过**：节点级回归覆盖暂拒、丢帧、同一目标重试及闸门不前移；`2026-08-25` 横向 G2 实跑 `10` 张／`2` 段，最大前后间距 `201.888 mm < 239.0625 mm`、横向 `412.615 mm < 425 mm`、相机位姿 `2.812 mm < 5 mm`。 |
| G4-06 | gate 禁用与存活监督 | 禁用段不得留下 active gate；巡检 SCAN 的首条／后续 gate 缺失时，机器人零速并在段超时前中止 | **通过**：首条 gate 允许一次性 `2.0 s` 建立窗口，后续心跳期限为 `0.50 s`；自动采集节点级覆盖 EKF 插值超时后的 inactive，执行器节点级覆盖无采集器时 `TRACKING_FAILED`，跨包测试固定 gate／归档／G2 重叠阈值关系。`2026-08-25` 横向 G2 实跑 `10` 张／`2` 段通过，证明正常启动不误触发。 |
