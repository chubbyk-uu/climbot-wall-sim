...
## Phase G2 位置触发与位姿绑定验收（实施中）

| ID | G2 要求 | 验收方法／门限 | 当前状态 |
| --- | --- | --- | --- |
| G2-01 | 仅正式扫描拍照 | 起点进入、ALIGN、转向稳定、TRANSITION、小弧线入轨和任务停止均为 0 张；只在 SCAN 直线产生图像 | 节点级门禁已实现，待 Gazebo 正式结果 |
| G2-02 | 前后重叠 | 首尾都有拍摄中心；同一扫描线曝光位置最大间距 ≤`0.225 m`，不重复编号、不漏编号 | 等间距与单调去重已实现，待动态实测 |
| G2-03 | 左右重叠与边界完整 | 相邻相机中心扫描线最大间距 ≤`0.400 m`；四边 20 mm 超覆；矩形横／竖和短顶边梯形按离散实际照片足迹覆盖率 ≥`98%` | 前置投影与超覆规划已实现，待三工况实测 |
| G2-04 | 图像与任务唯一绑定 | 每张图像恰有一条同时间戳元数据；任务 ID／版本、段号和触发号正确，取消／重启不串任务 | 接口与状态机已实现，待集成故障测试 |
| G2-05 | 曝光时刻融合位姿 | EKF 缓存前后插值，不允许最近样本替代；相机中心对真值误差 ≤`5 mm`，航向误差沿用定位验收门限 | 插值及杠杆协方差已实现，待 Gazebo 正式结果 |
| G2-06 | 相机前置偏移语义一致 | 规划、执行参考、触发、覆盖评价均使用 `+0.300 m`；偏移与安装外参不一致时明确拒绝自动采集 | 实现中，纯几何门禁待完成 |
