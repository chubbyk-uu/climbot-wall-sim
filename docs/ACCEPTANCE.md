...
| G4-05 | 正常采图不调制扫描速度 | 正常触发、图像和 EKF 位姿异步配对；持续 heartbeat 的长 SCAN 必须保持巡航速度。服务拒绝、图像超时或位姿插值失败不得在已越过目标后补拍，必须转为受控中止 | **待复验**：节点级回归覆盖持续 heartbeat 下的巡航速度、服务拒绝、丢帧和位姿超时后的 heartbeat 停止；需重跑 G2 与实际采集任务确认。 |
| G4-06 | gate 存活监督 | 巡检 SCAN 的首条／后续 heartbeat 缺失时，机器人零速并在段超时前中止；当前协议拒绝 `active=true` 的位置栅栏 | **待复验**：首条 heartbeat 允许一次性 `2.0 s` 建立窗口，后续期限为 `0.50 s`；执行器节点级覆盖无采集器时 `TRACKING_FAILED`。 |
