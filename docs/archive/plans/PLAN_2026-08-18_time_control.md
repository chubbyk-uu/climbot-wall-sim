...
**执行结果见文末第 8 节；第 3 节的设计结论已全部实测验证，第 7 节的风险已闭合。**
...
```yaml
tracking_mode: time
time_profile_acceleration: 0.20      # 对称，等于额定加速度
time_profile_deceleration: 0.20
time_speed_lag_s: 0.08               # 加速度前馈，实测 0.0799 s，见 8.7
time_along_gain: 2.5                 # 纯 P，见 8.8
time_along_integral_gain: 0.0        # 不需要，巡航段无稳态速度误差
catch_up_max_linear_speed: 0.35
catch_up_max_linear_acceleration: 0.35
time_mode_final_approach_enabled: true
time_axis_stretch_enabled: false     # 从未触发
schedule_align_converge_s: 1.24      # 时长模型标定，见 8.4
...
| 用例 | 落点 mm | 转向 ° | 道间距 mm | 覆盖 % | act/plan | lag s |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| bigV | 3.89 | 0.894 | 4.85 | 100.00 | 0.997 | 0.04 |
| bigTV | 3.02 | 0.907 | 5.50 | 98.98 | 1.002 | 0.04 |
| vertical | 2.96 | 0.872 | 2.19 | 100.00 | 1.015 | 0.04 |
| trapezoid_vertical | 3.86 | 0.965 | 4.34 | 98.63 | 1.001 | 0.05 |
| bigH | 2.94 | 0.880 | 2.77 | 99.97 | 1.005 | 0.03 |
| bigTH | 3.05 | 0.900 | 2.58 | 99.34 | 1.013 | 0.03 |
| trapezoid_horizontal | 3.69 | 0.994 | 4.66 | 99.75 | 1.002 | 0.04 |
| horizontal | 2.64 | 0.763 | 1.51 | 99.93 | 0.981 | 0.04 |

落点 `2.64~3.89 mm`、道间距 `1.51~5.50 mm`、转向落点航向 `0.763~0.994°`、
覆盖率 `98.63~100%`、`act/plan` `0.981~1.015`、峰值滞后 `0.03~0.05 s`。
`timeD`（`gain=3.0`）为同一配置的前一版，落点上界为 `4.59 mm`。

> **`timeE` 已被 `2026-08-19t` 取代。** 这一轮产自带未提交改动的工作树
...
八个验收用例，每轮全部通过。落点误差 / 道间距误差，单位 mm：

| 用例 | distance（`ceilgate`） | time 裸算法（`timeA`） | time + 兜底（`timeB`） |
| --- | --- | --- | --- |
| bigV | 3.88 / 5.36 | 15.23 / 17.60 | **3.31 / 4.64** |
| bigTV | 5.04 / 6.34 | 15.38 / 11.99 | **3.15 / 5.11** |
| vertical | 3.45 / 3.72 | 15.08 / 8.18 | **3.24 / 2.51** |
| trapezoid_vertical | 3.24 / 3.15 | 15.33 / 7.72 | **3.26 / 3.49** |
| bigH | 2.60 / 3.57 | 13.26 / 8.84 | **2.65 / 3.56** |
| bigTH | 2.67 / 2.34 | 10.98 / 5.49 | **3.42 / 2.85** |
| trapezoid_horizontal | 2.97 / 6.21 | 10.87 / 9.49 | **2.67 / 3.56** |
| horizontal | 2.87 / 1.88 | 11.98 / 6.41 | **2.50 / 2.98** |

验收限值：落点 ≤ 30 mm，转向落点航向 ≤ 2.0°，道间距 ≤ 20 mm，覆盖率 ≥ 95%。
转向落点航向三轮均在 0.75~1.01° 之间——转向那条路本来就没改动，这正是预期。

任务时长（仿真秒）：time + 兜底比 distance 一致快 0.2~1.9%，来自对称制动。

### 8.2 阶段 1 门禁：抬高上限没有改变 distance 模式

八个用例的**覆盖率全部保持到小数点后两位完全一致**，落点四升四降、转向四升四降，
无系统性偏移。门禁通过。

### 8.3 裸算法的失效模式已定位

`timeA` 的落点误差 10.9~15.4 mm 与峰值滞后 0.08 s 完全对应
（0.08 s × 0.20 m/s = 16 mm）。解析轨迹后确认机制：

- **滞后几乎全部在第一秒内建立**（`lag@1s` 已达 9~15 mm，之后基本不再增长）。
  profile 假设机器人从静止按额定 `0.20 m/s²` 起步，实际起步差约 12%。
- **P 环随后过度补偿**：到段末机器人反而**超前**自己的时间表 7~13 mm。

所以落点误差是**超冲而非欠冲**。第 3.7 节的兜底之所以只需交出最后 50 mm 就够，
正是因为把停车判据换回位置的函数后，携带的动量和残余修正不再参与停车决策。

### 8.4 时长模型标定完成，预测精度 1.5% 量级

把八条基线轨迹按控制状态拆开，与模型逐项对比，缺口是三个稳定的每段常数：

| 项 | 每段超出量 | sd | 对应参数 |
| --- | --- | --- | --- |
| `ALIGN_SETTLE` 收敛到 1° 死区 | 1.240 s | 0.144 | `schedule_align_converge_s` |
| `ALIGN` 段（含等待上一条指令衰减） | 0.427 s | 0.168 | `schedule_handshake_s` |
| 终点限速 + 减速到静止 | 0.130 s | 0.057 | `schedule_goal_stop_s` |

三项之和 1.80 s/段。代入后预测精度由 **+10.6%（sd 2.4%）** 变为
**−0.4%（sd 1.2%）**，最差 3.0%（`horizontal`，段数最少所以每段项占比最大）。
`timeB` 实测 `act/plan` 落在 **0.977~1.015**，均值 1.003。

第 3.10 节设定的 2% 目标达成。主项是 `ALIGN_SETTLE` 收敛而非任何一个"保持"常数——
这正是把单一 `settle_duration` 常数拆开的价值：拆开之前它只能被整体拟合。

### 8.5 与预期不符之处

- **梯形并不比矩形显著更差。** 3.10 节推测梯形因斜向换道转向更多而开销更大，
  实测每段缺口梯形 2.89 s、矩形 2.51 s，区间重叠，不构成分类依据。真正的
  区分度在段数与扫描线长度上，而三个每段常数已经把它吸收掉了。
- **"横向调整航向角"不是额外等待时间**（3.10 节已澄清）：`trackLine()` 返回的
  `heading_error` 已含重力前馈，`ALIGN_PROFILE` 对准的就是含前馈的航向。

### 8.6 仍然开着的问题

- ~~`tracking_mode` 默认仍是 `distance`~~ **已于 2026-08-19 切为 `time`**，依据是
  同一提交同一棵干净树上的 `2026-08-19` / `2026-08-19t` 两组八工况对照：落点、
  道间距、转向落点航向、覆盖率没有系统性差异，任务时长预测 `time` 优于 `distance`
  （sd `0.92%` 对 `1.43%`，最大偏离 `1.96%` 对 `3.18%`），且 `time` 逐拍上报自身
  滞后。运行时仍可从面板或 `ros2 param set` 切回。
- ~~标定值写入配置后尚未跑过 distance 模式的回归~~ **已补**：`2026-08-19` 八工况
  实测 `act/plan` `0.968~1.016`，8.4 的离线拟合不再是唯一依据。
- 起步 12% 的加速亏欠是 profile 与实际执行的模型误差，目前由 P 环和终点兜底
  共同吸收。更彻底的做法是按实测起步特性规划 profile，或让时间轴在机器人真正
  起步时才开始计时；本轮不做。
- `time_axis_stretch_enabled` 仍为 `false`，未在任何一轮触发过（峰值滞后 0.09 s，
  远低于 0.05 m 阈值对应的量级）。保留待真机打滑更严重时启用。
- ARC_ENTRY 在三轮回归中均未进入（转向落点 2.6~3.4 mm，远在 45 mm 分界之内），
  其与 time 模式的兼容由 `test_coverage_arc_entry.py` 与单段测试覆盖，
  未获得整任务级的实证。

### 8.7 起步亏欠的成因与加速度前馈

`timeA` 的起步亏欠**不是加速度权限不足**。逐拍对比指令与真值速度：

```
 t_s    cmd_mps  truth_mps    ratio    a_cmd
0.10     0.0218     0.0045    0.207    0.218
0.30     0.0614     0.0453    0.737    0.200
0.50     0.1020     0.0888    0.871    0.167
0.70     0.1483     0.1301    0.878    0.262
1.00     0.2084     0.1929    0.926    0.217
1.10     0.2071     0.2075    1.002   -0.013   ← 追平
```

`a_cmd` 全程都是额定的 `0.20 m/s²`，限幅器（`0.35`）没有削它；真值速度一路落后到
`1.1 s` 才追平。这是**执行机构速度环的一阶滞后**。在匀加速段
`亏欠速度 = τ · a`，据此在 `timeB` 的 **124 个段**上测得：

| 用例 | τ (s) | 用例 | τ (s) |
| --- | ---: | --- | ---: |
| bigH | 0.0850 | bigTH | 0.0839 |
| bigTV | 0.0787 | bigV | 0.0761 |
| horizontal | 0.0851 | trapezoid_horizontal | 0.0861 |
| trapezoid_vertical | 0.0761 | vertical | 0.0772 |

**整体 τ = 0.0799 s（sd 0.0193, n=124）**，横向 `0.084~0.086` 略高于竖向
`0.076~0.077`，与横向行走要侧着抗重力一致。
`τ × 0.20 m/s² = 16.0 mm/s`，加速段 1 秒 → 亏欠 **16 mm**，与实测滞后完全吻合。

用增益压这个误差是错的：P 只能在误差**已经产生之后**反应，而事后偿还正是把机器人
推过段末的原因。一阶滞后的解法是把曲线的**加速度**一并前馈：
`v_cmd = v(t) + τ·v̇(t) + K·滞后`。制动段 `v̇` 为负，指令自动提前减速，方向同样正确。

实测效果（同样 124 个段，`timeB` → `timeC`）：

| 量 | 无前馈 | 有前馈 | 变化 |
| --- | ---: | ---: | ---: |
| 加速段结束时滞后 | 10.8 mm | 1.1 mm | **−90%** |
| 整段峰值滞后 | 13.9 mm | 6.8 mm | −51% |

更有说服力的是残余滞后的**位置分布**：前馈之前峰值集中在段首，之后均匀分布在全段
（0.30~0.85 区间占 57%）。系统性误差已清除，剩下的是巡航中的随机扰动。

### 8.8 P 增益：从两端定值，不靠经验

在 `bigH / vertical / trapezoid_horizontal / horizontal` 四个用例上扫四个梯度。
汇总表的 `lag_s` 只有 `0.01 s` 分辨率，不足以区分相邻梯度，因此峰值滞后从轨迹
CSV 逐段重算（mm 级，n=46 段）：

| gain | vertical | 峰值滞后 mm | p90 mm | 指令抖动 mean·\|dv/dt\| |
| ---: | --- | ---: | ---: | ---: |
| 0.0 | **超时失败**，174 mm / 覆盖 14.6% | — | — | — |
| 1.0 | 通过 | 7.06 | 11.63 | 0.0258 |
| **2.5** | 通过 | **5.11** | **7.66** | 0.0658 |
| 3.0 | 通过 | 5.78 | 7.82 | 0.0694 |
| 6.0 | 通过 | 6.20 | 7.91 | 0.1463 |

两端都给出了硬结论：

- **P = 0 直接失败**。线速度通道纯开环，竖向爬升被重力吃掉沿轨距离，曲线走完时
  机器人仍未到位，指令归零就地停死，`segment_timeout_s` 超时，
  `result_code=4 CONTROL_TIMEOUT`，`completed_segments=0`。横向三例能过，是因为
  横向下滑主要在横轨方向，机器人仍能进入终点 `50 mm` 内被距离制动兜底接管。
  **P 项是让段能够结束的东西，不是修饰。**
- **峰值滞后不是随增益单调下降的**：在 `2.5` 附近取到极小，`3.0` 和 `6.0` 反而
  回升——增益再硬就开始过度修正。而指令抖动是**严格单调上升**的
  （`0.026 / 0.066 / 0.069 / 0.146`）。

逐段配对比较（同用例同段号，n=46）：

| 对比 2.5 | 平均改善 | 2.5 更好 | sd | t |
| --- | ---: | ---: | ---: | ---: |
| vs 1.0 | 1.94 mm | 36/46 | 2.51 | 5.2 |
| vs 3.0 | 0.67 mm | 32/46 | 1.88 | 2.4 |
| vs 6.0 | 1.09 mm | 36/46 | 1.78 | 4.2 |

分用例看 `2.5` 与 `3.0` 互有胜负（`2.5` 赢在 `vertical` 与 `trapezoid_horizontal`，
`3.0` 赢在 `bigH` 与 `horizontal`），差距在噪声边缘。**同等表现下取更低的增益**：
滞后这一轴分不出高下，抖动这一轴是明确的，所以定 **P = 2.5**。

选它的依据不是验收指标——那几项在 `P=1` 就已经贴地板了——而是把 `P=0` 暴露出的
那个唯一已知失效模式的抗扰余量翻倍。

一次 `-m time` 的 `trapezoid_horizontal` 在 `P=3` 时失败过，查明是**仿真侧故障**：
真值位姿 120 s 完全不变（`(0, 1.9996, yaw 0)` 四位小数），而控制器一直在发满量程
`−0.6 rad/s`。该 lane 的机器人从未响应任何指令，重跑即通过（`2.84 mm`）。
与增益无关，记此备查。

### 8.9 不需要 I 项

巡航段真值速度与指令速度之比稳定在 `1.00`（见 8.7 的表），没有稳态速度误差可供
积分消除；`timeA` 段末的偏差是 P 的超冲而非稳态残差。`time_along_integral_gain`
保持 `0.0`，实现保留待真机出现稳态误差时启用。
